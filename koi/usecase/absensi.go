package usecase

import (
	"context"
	"koi-backend-web-go/domain"
	"time"
)

type absensiUseCase struct {
	absensiRepository domain.AbsensiRepoository
	contextTimeout    time.Duration
}

func NewAbsensiUseCase(dtl domain.AbsensiRepoository, t time.Duration) domain.AbsensiUseCase {
	return &absensiUseCase{
		absensiRepository: dtl,
		contextTimeout:    t,
	}
}

func (a *absensiUseCase) CreateAbsensi(ctx context.Context, req *domain.Absensi) (*domain.Absensi, error) {
	return a.absensiRepository.CreateAbsensi(req)
}

func (a *absensiUseCase) GetAbsensiByEventID(ctx context.Context, eventId uint) ([]domain.Absensi, error) {
	return a.absensiRepository.GetAbsensiByEventID(eventId)
}

func (a *absensiUseCase) UpdateStatus(ctx context.Context, eventId uint, userId uint, status string) error {
	return a.absensiRepository.UpdateStatus(eventId, userId, status)
}

func (a *absensiUseCase) GetAllAbsensi(ctx context.Context) ([]domain.Absensi, error) {
	return a.absensiRepository.GetAllAbsensi()
}
